...
public class Main
{
    ...
    private IList<GraficoDto> GetDatos(string baseDatos, string nombre, string fechaMin, string fechaMax)
    {
        ...
        if (nombre == "0")
        {
            ...
            OC,DCC,SE,0 as 'EnergiaAsignadaOV',
            PerfilPorcentual*PotenciaConEnergiaAsociada as 'EnergiaLicitacion'
            FROM 
            (SELECT t0.A,t0.Mes,T0.ID_Mes,ID_Hora,
            sum(if(t0.id_contrato='OC',potenciamaxima,0)) as 'OC',
            sum(if(t0.id_contrato='DCC',PerfilPorcentual*potenciamaxima,0)) as DCC,
            sum(if(t0.id_contrato='SE',PerfilPorcentual*potenciamaxima,0)) as SE
            from `{0}`.mod_oferentePerfil as t0
                inner join (select * from `{0}`.mod_oferente) as t3 on t0.ID_Oferente=t3.ID_Oferente 
                inner join `{0}`.mod_oferentemes as t6 on t0.Id_Mes=T6.ID_Mes and t0.ID_Oferente=t6.ID_Oferente
                group by A,Mes,ID_Hora) t2
        ...
        else
        {
            ...
            sum(if(t0.id_contrato='SE',PerfilPorcentual*potenciamaxima,0)) as SE
            from `{0}`.mod_oferentePerfil as t0
                inner join (select * from `{0}`.mod_oferente where Nombre= :Nombre) as t3 on t0.ID_Oferente=t3.ID_Oferente 
                inner join `{0}`.mod_oferentemes as t6 on t0.Id_Mes=T6.ID_Mes and t0.ID_Oferente=t6.ID_Oferente
                group by A,Mes,ID_Hora) t2
                inner join `{0}`.mod_licitacionmes as t4 on t2.Id_Mes=T4.ID_Mes
                inner join `{0}`.mod_licitacionperfil as t5 on t2.Id_Mes=T5.ID_Mes and t2.ID_Hora=t5.ID_Hora
    ...
    public object ObtenerGraficos(string baseDatos, string nombre, string fechaMin, string fechaMax)
    {
        ...
        try
        {
            ...
            var label = "DCC";
            charts.Add(new ChartDto { key = label, yAxis = 1, type = ChartDto.TYPE_AREA, color = "#73a8eb", order = 1 });
            for (int i = 0; i < (result.Count); i += factor)
            {
                Double Value = 0;
                for (int j = i; j < i + factor; j++)
            ...
            label = "EnergiaAsignadaOV";
            charts.Add(new ChartDto { key = label, yAxis = 1, type = ChartDto.TYPE_AREA, color = "#696969", order = 4 });
            for (int i = 0; i < (result.Count); i += factor)
            {
                Double Value = 0;
                Value = result[i].EnergiaAsignadaOV;
                charts[charts.Count - 1].values.Add(new ChartValuesDto { x = i , y = Math.Round(Value, 2) });
            }
    ...
    public class GraficoDto
    {
        ...
        public double EnergiaAsignadaOV { get; set; }
